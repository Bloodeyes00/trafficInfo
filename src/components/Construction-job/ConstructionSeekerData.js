...
function FactorySeekerData() {
  ...
  const updateData = (e) => {
    e.preventDefault();
    db.collection("ConstructionSeeker").doc(dataIdToBeUpdated).update({
      email: updatedEmail,
  ...
  useEffect(() => {
    db.collection("ConstructionSeeker").onSnapshot((snapshot) => {
      setData(
    ...
    const deleteData = (item) => {
      ...
      if (firebase.auth().currentUser.uid == item.data?.posterdID) {
        db.collection("ConstructionSeeker").doc(id). delete().then(() => {
      ...
      {data.map((item, index) => (
        <div className='row-taxijob ms-3'>
          ...
          <div className='d-flex'>
            ...
            <h2 className='rowtext1'  style={{fontSize:"15px",fontWeight:"bold",color:"#ffcc00",width:"200px"}}>MECHANIC</h2>
            </div>
...
export default FactorySeekerData
